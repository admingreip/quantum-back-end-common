...
public class UtlGeneral {
	...
	public static Logger log = LoggerFactory.getLogger(UtlGeneral.class);
	
	/**
	 * Carga las propiedades de un archivo de configuracion externo. Toma la variable de entorno SYSDE_WS_HOME
	 * @param springApplicationName nombre de la aplicacion
	 * @return java.util.Properties
	 */
	public static Properties getConfigProperties(String springApplicationName) {
		String configLocation = System.getenv("GREIP_WS_HOME"); // get the default config directory location
		String configPath = configLocation +File.separator+ "config"+File.separator+springApplicationName+".properties";					
		if (configLocation != null && !configLocation.isEmpty()) {
			log.info(String.format(ConstantesCore.SpringBootMessages.LOAD_APP_LOADING, springApplicationName, configPath));
			Properties props = new Properties();
			props.setProperty("spring.config.location", configPath); // set the config file to use
			return props;
		} else {
			log.error(String.format(ConstantesCore.SpringBootMessages.LOAD_APP_ERROR, springApplicationName));
			return null;
		...
		for (Field f : atributos) {
			...
			if (f.getType().toString().equals("class java.lang.String")) {
				...
				try {
					...
					metodoGet = cls.getMethod(
							"get" + f.getName().substring(0, 1).toUpperCase() + f.getName().substring(1), null);
					metodoSet = cls.getMethod(
