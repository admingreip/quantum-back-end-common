...
public class UtlSecurity {
	...
	/***
	 * Encriptacion para OauthClientDetails (base de datos oauth_client_details)
	 * @param password
	 * @return password encriptado
	 */
	public static String oauthClientPasswordEncoder(String password) {
		String encode = "";
		try {
	...
	/***
	 * Encriptacion para OauthUsers (base de datos se_usuario)
	 * @param password
	 * @return password encriptado
	 */
	public static String userPasswordEncoder(String password) {
		String encode = "";
		try {
	...
	public static void encriptarArchivo(String archivo) {
		try {
			...
			fos = new FileOutputStream(desFile.getParent() + desFile.getName() + ".sion");
			byte[] b = new byte[8];
			int i = cis.read(b);
			while (i != -1) {
				fos.write(b, 0, i);
			...
			javax.swing.JOptionPane.showMessageDialog(null, "Imposible abrir el archivo!!!, o nombre incorrecto");
		}
	}

	public static void desencriptarArchivo(String archivoEncriptado, String archivoDesencriptado) {
		try {
	...
	private static final byte[] SALT_BYTES = { (byte) 0xA9, (byte) 0x9B, (byte) 0xC8, (byte) 0x32, (byte) 0x56,
	...
	public static String encriptarTexto(String textoDesencriptado) {
		...
		try {
			...
			return Base64.getEncoder().encodeToString(oEnc);
		} catch (Exception oE1) {
			oE1.printStackTrace();
		}
		return null;
	...
	public static String desencriptarTexto(String textoEncriptado) {
		...
		try {
			...
			byte[] oDec = Base64.getDecoder().decode(sStringValue);	
			// Desencriptar
